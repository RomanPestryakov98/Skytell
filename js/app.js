(() => {
    ...
    function menuInit() {
        if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
            if (bodyLockStatus && e.target.closest(".icon-menu")) {
                ...
                document.querySelector(".top-menu__menu").classList.toggle("_active");
            } else if (bodyLockStatus && e.target.closest(".top-menu__close")) {
                bodyLockToggle();
    ...
    function eventTime(time) {
        ...
        let curTime = "";
        if (time < 60) if (time < 10) curTime = `00:0${curentTime}`; else curTime = `00:${curentTime}`; else {
            const minutes = Math.floor(curentTime / 60);
            const seconds = curentTime % 60;
            if (minutes <= 9 && seconds <= 9) curTime = `0${minutes}:0${seconds}`; else if (minutes <= 9 || seconds > 9) curTime = `0${minutes}:${seconds}`;
        }
        return curTime;
    }
